...
def parse_optical_config_file(config_file_path, instrument_config):
...
def parse_observation_config_file(config_file_path, user_config, instrument_config):
    with open(config_file_path, 'r') as f:
        for line in f:
            if(line[0] == '#'):
                continue
            ...
            if(values[0] == "filter"):
                filter_location = FILTERS[instrument_config.filter_list].get(values[1], "")
                instrument_config.add_filter(values[1], filter_location)
                continue

            if(values[0] == "apparent_magnitude"):
                user_config.object_magnitude = float(values[1]) * units.ABmag
                if(DEBUG): print("DEBUG: Apparent Magnitude = ", user_config.object_magnitude)
                continue

            if(values[0] == "wavelength"):
                user_config.observed_wavelength = units.micrometer * float(values[1])
                if(DEBUG): print("DEBUG: Observed Wavelength = ", user_config.observed_wavelength)
                continue

            if(values[0] == "width"):
                user_config.OBSERVATION_MODE == "imaging"
                user_config.filter_band_width = (units.micrometer) * float(values[1])
                if(DEBUG): print("DEBUG: Filter width = ", user_config.filter_band_width)
                continue

            if(values[0] == "exposure_time"):
                user_config.exposure_time = units.second * float(values[1])
                if(DEBUG): print("DEBUG: Exposure Time = ", user_config.exposure_time)
                continue
    
            if(values[0] == "number_of_exposures"):
                user_config.number_of_exposures = int(values[1])
                if(DEBUG): print("DEBUG: Number of exposures = ", user_config.number_of_exposures)
                continue

            if(values[0] == "sky_mag"):
                user_config.sky_mag = units.ABmag * float(values[1])
                if(DEBUG): print("DEBUG: Sky Contribution = ", user_config.sky_mag)
                continue

            if(values[0] == "airmass"):
                user_config.airmass = float(values[1])
                if(DEBUG): print("DEBUG: Airmass = ", user_config.airmass)
                continue
...
class InstrumentConfiguration:
    ...
    #Uses a configuration file to setup all the currently installed mirrors and stuff on the telescope.
        ...
        self.m_element_list = []
        parse_optical_config_file(config_file, self)
...
def user_input():
    ...
    #Automatic input of observation configuration
    if(OBSERVATION_FILE != ""):
        parse_observation_config_file(OBSERVATION_FILE, user_config, instrument_config)
        return user_config, instrument_config

    #Manual input of Observation configuration
    filter_input = False
...
def parse_command_line():
    ...
    while(len(args) >= 1):
        ...
        if(arg == "-c" or arg == "--c" or arg == "--config"):
            arg = args.pop()
            
            if(arg.startswith("-") or arg.startswith("--")):
                print("Missing config file path, type --help for help")
                exit()
        ...
        global OBSERVATION_FILE
        OBSERVATION_FILE = arg



def print_help():
    print("Usage: ./gmt_etc.py [-h] [-d] [-c config_file] [observation_config")
    print("Options:")
    print(" -h, --help, -?      Prints this help message.")
    print(" -d, --debug         Enables debug messages.")
    print("")
    print(" -c, --config config_file The path of the optical path config")
    print("                          file to use, will default to \"optical_config\"")
    print("")
    print(" observation_config  The path of the observation configuration file")
    print("                     to use")
main()
